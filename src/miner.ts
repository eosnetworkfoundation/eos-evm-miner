...
export default class EosEvmMiner {
    ...
    async eth_sendRawTransaction(params: any[]) {
        ...
        this.pushCount++;
        logger.info(
            `Pushing tx #${this.pushCount} to ${this.config.rpcEndpoints[this.currentRpcEndpoint]}`
        );
        ...
        await this.api
            ...
            .catch((e) => {
                logger.error(
                    `Error pushing #${this.pushCount} to ${
                        this.config.rpcEndpoints[this.currentRpcEndpoint]
                    }`
                );
                logger.error(e);
    ...
    async eth_gasPrice() {
        ...
        if (!(this.config.lockGasPrice && this.gasPrice !== '0x1') && timeSinceLastCall >= 1000) {
            try {
                ...
                throw new Error(
                    'There was an error getting the gas price from this EOS EVM miner.'
                );
            }
        }
